...
from ashare_daily.pool.builder.clustering import (
...
def test_return_scores_cumulative_return_picks_highest_total(monkeypatch):
    ...
    monkeypatch.setattr("ashare_daily.pool.builder.clustering.SCORING_MODE", "cumulative_return")
    returns = _toy_returns()
    members = ["HIGH", "MID", "LOW"]
...
def test_return_scores_cumulative_return_respects_scoring_mode_env(monkeypatch):
    ...
    monkeypatch.setattr("ashare_daily.pool.builder.clustering.SCORING_MODE", "sharpe_mom")
...
def test_global_cross_cluster_scores_low_corr_return_balances_return(monkeypatch):
    ...
    monkeypatch.setattr("ashare_daily.pool.builder.clustering.CROSS_LOW_CORR_WEIGHT", 0.5)
    idx = pd.date_range("2020-01-01", periods=60, freq="B")
    n = len(idx)
...
def test_plan_b_orders_by_global_cumulative_return(monkeypatch):
    ...
    monkeypatch.setattr("ashare_daily.pool.builder.clustering.CROSS_CLUSTER_RANK", "cumulative_return")
    idx = pd.date_range("2020-01-01", periods=60, freq="B")
    returns = pd.DataFrame(
        {
